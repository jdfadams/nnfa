...
// I used the function below when testing the neural network.
// This function is not as useful as the function plotfn.
/*
...
void plotfn(
 ...
 cout << "The target function is drawn in blue, and the neural network's interpretation is drawn in red." << endl
      << "The bitmap file \"" << fname << "\" has been saved." << endl;
 return;
}
...
void test(double (*func)(const double &), const double &a, const double &b, const unsigned int &N, const unsigned int &T, const unsigned int &C, const char *fname)
{
 ...
 // Build a training set consisting of T distinct pairs (x, func(x)), where x is chosen randomly from the interval [a, b].
 // Alternatively, one might choose equally spaced points in the interval.
 while (X.size() < T)
 {
 ...
 // Train the neural network by passing over the training set C times.
 for (i = 0; i < C; i ++)
 {
...
// This is the function we teach our neural network.
